package com.simibubi_port.create.build.tasks;
...
import com.simibubi_port.create.build.UfoPortBuildPlugin;
import com.simibubi_port.create.build.Utils;
...
public class FmjExpander extends FilterReader {
	...
	public static final String TEMPLATE_FMJ = "gradle/template." + FMJ;
	...
	private void expand() throws IOException {
		try {
			...
			String asString = UfoPortBuildPlugin.GSON.toJson(expanded);
	...
	public JsonObject expandFmj(JsonObject moduleFmj) throws IOException {
		...
		try (Stream<Path> files = Files.list(resources)) {
			List<Path> glist = files.filter(file -> file.toString().endsWith(".mixins.json")).toList();
			for (Path mixins : glist) {
				if (Files.exists(mixins)) {
					String mixinsFileName = mixins.getFileName().toString();
					mixinsFound++;
