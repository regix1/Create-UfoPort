...
/**
 * A builder for blocks, allows for customization of the
 * {@link Block.Properties}, creation of block items, and configuration of data
 * associated with blocks (loot tables, recipes, etc.).
 * 
...
public class BlockBuilder<T extends Block, P> extends AbstractBuilder<Block, T, P, BlockBuilder<T, P>> {
	...
	/**
	 * Create a new {@link BlockBuilder} and configure data. Used in lieu of adding
	 * side-effects to constructor, so that alternate initialization strategies can
	 * be done in subclasses.
	 * <p>
	 * The block will be assigned the following data:
	 * <ul>
	 * <li>A default blockstate file mapping all states to one model (via
	 * {@link #defaultBlockstate()})</li>
	 * <li>A simple cube_all model (used in the blockstate) with one texture (via
	 * {@link #defaultBlockstate()})</li>
	 ...
	 */
	public static <T extends Block, P> BlockBuilder<T, P> create(AbstractRegistrate<?> owner, P parent, String name,
			BuilderCallback callback, NonNullFunction<BlockBehaviour.Properties, T> factory) {
	...
	private NonNullFunction<BlockBehaviour.Properties, BlockBehaviour.Properties> propertiesCallback = NonNullUnaryOperator
			.identity();
	private Supplier<Supplier<RenderType>> renderType;
	@Nullable
	private NonNullSupplier<Supplier<BlockColor>> colorHandler;

	protected BlockBuilder(AbstractRegistrate<?> owner, P parent, String name, BuilderCallback callback,
	...
	/**
	 * Modify the properties of the block. Modifications are done lazily, but the
	 * passed function is composed with the current one, and as such this method can
	 * be called multiple times to perform different operations.
	 * <p>
	 * If a different properties instance is returned, it will replace the existing
	 * one entirely.
	 * 
	...
	/**
	 * Replace the initial state of the block properties, without replacing or
	 * removing any modifications done via
	 * {@link #properties(NonNullUnaryOperator)}.
	 * 
	 * @param block The block to create the initial properties from (via
	 *              {@link Block.Properties#copy(BlockBehaviour)})
	 * @return this {@link BlockBuilder}
	 */
	...
	protected void registerLayers(T entry) {
		EnvExecutor.runWhenOn(EnvType.CLIENT,
				() -> () -> BlockRenderLayerMap.INSTANCE.putBlock(entry, renderType.get().get()));
	}

	/**
	 * Create a standard {@link BlockItem} for this block, building it immediately,
	 * and not allowing for further configuration.
	 * <p>
	 * The item will have no lang entry (since it would duplicate the block's) and a
	 * simple block item model (via
	 * {@link RegistrateItemModelProvider#blockItem(NonNullSupplier)}).
	 *
	 * @return this {@link BlockBuilder}
	 * @see #item()
	 */
	...
	/**
	 * Create a standard {@link BlockItem} for this block, and return the builder
	 * for it so that further customization can be done.
	 * <p>
	 * The item will have no lang entry (since it would duplicate the block's) and a
	 * simple block item model (via
	 * {@link RegistrateItemModelProvider#blockItem(NonNullSupplier)}).
	...
	/**
	 * Create a {@link BlockItem} for this block, which is created by the given
	 * factory, and return the builder for it so that further customization can be
	 * done.
	 * <p>
	 * By default, the item will have no lang entry (since it would duplicate the
	 * block's) and a simple block item model (via
	 * {@link RegistrateItemModelProvider#blockItem(NonNullSupplier)}).
	 * 
	 * @param <I>     The type of the item
	 * @param factory A factory for the item, which accepts the block object and
	 *                properties and returns a new item
	 * @return the {@link ItemBuilder} for the {@link BlockItem}
	 */
	...
	/**
	 * Create a {@link BlockEntity} for this block, which is created by the given
	 * factory, and assigned this block as its one and only valid block.
	 * 
	...
	/**
	 * Register a block color handler for this block. The {@link BlockColor}
	 * instance can be shared across many blocks.
	 * 
	 * @param colorHandler The color handler to register for this block
	 * @return this {@link BlockBuilder}
	 */
	// TODO it might be worthwhile to abstract this more and add the capability to
	// automatically copy to the item
	...
	/**
	 * Assign the default blockstate, which maps all states to a single model file
	 * (via {@link RegistrateBlockstateProvider#simpleBlock(Block)}). This is the
	 * default, so it is generally not necessary to call, unless for undoing
	 * previous changes.
	 *
	 * @return this {@link BlockBuilder}
	 */
	 ...
	 * {@link RegistrateLangProvider#getAutomaticName(NonNullSupplier, net.minecraft.resources.ResourceKey)}.
	 * This is the default, so it is generally not necessary to call, unless for
	...
	/**
	 * Assign the default loot table, as specified by
	 * {@link RegistrateBlockLootTables#dropSelf(Block)}. This is the default, so it
	 * is generally not necessary to call, unless for undoing previous changes.
	 * 
	 * @return this {@link BlockBuilder}
	 */
	...
	/**
	 * Configure the loot table for this block. This is different than most data gen
	 * callbacks as the callback does not accept a {@link DataGenContext}, but
	 * instead a {@link RegistrateBlockLootTables}, for creating specifically block
	 * loot tables.
	 * <p>
	 * If the block does not have a loot table (i.e.
	 * {@link Block.Properties#noLootTable()} is called) this action will be
	 * <em>skipped</em>.
	 * 
	 * @param cons The callback which will be invoked during block loot table
	 *             creation.
	 * @return this {@link BlockBuilder}
	 */
	...
	/**
	 * Assign {@link TagKey}{@code s} to this block. Multiple calls will add
	 * additional tags.
	 * 
