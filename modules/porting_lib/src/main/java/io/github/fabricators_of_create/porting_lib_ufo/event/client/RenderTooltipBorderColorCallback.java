...
public interface RenderTooltipBorderColorCallback {
	Event<RenderTooltipBorderColorCallback> EVENT = EventFactory.createArrayBacked(
			RenderTooltipBorderColorCallback.class,
			callbacks -> (stack, originalBorderColorStart, originalBorderColorEnd) -> {
				for (RenderTooltipBorderColorCallback callback : callbacks) {
					BorderColorEntry entry = callback.onTooltipBorderColor(stack, originalBorderColorStart,
							originalBorderColorEnd);
					if (entry != null) {
						return entry;
					}
				}
