...
public abstract class ScrollPanel extends AbstractContainerEventHandler implements Renderable, NarratableEntry {
	...
	protected void drawBackground(GuiGraphics matrix, Tesselator tess, float partialTick) {
		if (this.client.level != null) {
			...
			//worldr.begin();
			worldr.addVertex(this.left,  this.bottom, 0.0f).setUv(this.left  / texScale, (this.bottom + (int)this.scrollDistance) / texScale).setColor(0x20, 0x20, 0x20, 0xFF);
			worldr.addVertex(this.right, this.bottom, 0.0f).setUv(this.right / texScale, (this.bottom + (int)this.scrollDistance) / texScale).setColor(0x20, 0x20, 0x20, 0xFF);
			worldr.addVertex(this.right, this.top,    0.0f).setUv(this.right / texScale, (this.top    + (int)this.scrollDistance) / texScale).setColor(0x20, 0x20, 0x20, 0xFF);
			worldr.addVertex(this.left,  this.top,    0.0f).setUv(this.left  / texScale, (this.top    + (int)this.scrollDistance) / texScale).setColor(0x20, 0x20, 0x20, 0xFF);
			MeshData data = worldr.build();
			if(data != null)
	...
	@Override
	public boolean mouseScrolled(double mouseX, double mouseY, double amount, double scroll) {
		if (scroll != 0) {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		if (extraHeight > 0) {
			...
			MeshData data3 = worldr.build();
			if(data3 != null)
				BufferUploader.drawWithShader(data3);
		}
