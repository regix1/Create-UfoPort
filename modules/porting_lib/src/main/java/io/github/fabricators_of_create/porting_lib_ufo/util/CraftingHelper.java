...
public class CraftingHelper {
	...
	public static Ingredient getIngredient(JsonElement json) {
		...
		return Ingredient.CODEC.parse(JsonOps.INSTANCE, json).result().get();
	}
	...
	public static ItemStack getItemStack(JsonObject json, boolean readNBT, boolean disallowsAirInRecipe) {
		...
		if (readNBT && json.has("nbt")) {
			...
			return ItemStack.parseOptional(PortingLibBase.getRegistryAccess(), tmp);
		}
	...
	public static boolean areShareTagsEqual(ItemStack stack, ItemStack other) {
		return stack.getComponents().equals(other.getComponents());
	}

	public static boolean checkConditionPredicate(JsonObject json) {
		ResourceLocation loc = ResourceLocation.parse(GsonHelper.getAsString(json, ResourceConditions.CONDITIONS_KEY));
		return ResourceConditions.getConditionType(loc).codec().codec().decode(JsonOps.INSTANCE, json).
			getOrThrow().getFirst().test(PortingLibBase.getRegistryAccess());
	}
	...
	public static boolean processConditions(JsonArray conditions) {
		for (int x = 0; x < conditions.size(); x++) {
			...
			if (!CraftingHelper.checkConditionPredicate(json))
				return false;
		}
		return true;
	}
