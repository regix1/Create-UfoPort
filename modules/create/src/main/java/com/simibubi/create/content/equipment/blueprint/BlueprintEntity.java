...
public class BlueprintEntity extends HangingEntity
	...
	public BlueprintEntity(EntityType<?> p_i50221_1_, Level p_i50221_2_) {
		...
		super.getDimensions(Pose.STANDING).withEyeHeight(0);
			...
			do {
				try (Transaction t = TransferUtil.getTransaction()) {
					...
					if (success) {
						...
						if (!recipe.isPresent()) {
							Optional<RecipeHolder<CraftingRecipe>> opt1 = level().getRecipeManager()
									.getRecipeFor(RecipeType.CRAFTING, craftingInventory.asCraftInput(), level());
							if(opt1.isPresent()) {
								recipe = Optional.of(opt1.get().value());
							}
						}
									
							
						ItemStack result = recipe.filter(r -> r.matches(craftingInventory.asCraftInput(), level()))
								.map(r -> r.assemble(craftingInventory.asCraftInput(), level().registryAccess()))
