...
public class SubMenuConfigScreen extends ConfigScreen {
	...
	public SubMenuConfigScreen(Screen parent, String title, ModConfig.Type type, ModConfigSpec configSpec, UnmodifiableConfig configGroup) {
		super(parent);
		this.type = type;
	...
	protected void resetConfig(UnmodifiableConfig values) {
		values.valueMap().forEach((key, obj) -> {
			if (obj instanceof AbstractConfig) {
				...
				ModConfigSpec.ValueSpec valueSpec = spec.getSpec().getRaw((List<String>) configValue.getPath());
