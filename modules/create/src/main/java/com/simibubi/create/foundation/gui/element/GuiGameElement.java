...
import io.github.fabricators_of_create.porting_lib_ufo.models.virtual.FixedColorTintingBakedModel;
...
//					LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY);
			...
			blockRenderer.getModelRenderer().tesselateBlock(VirtualEmptyBlockGetter.FULL_BRIGHT, model, blockState,
					BlockPos.ZERO, ms, vb, false, RandomSource.create(), 42L, OverlayTexture.NO_OVERLAY);
			buffer.endBatch();
		}
...
//				buffer.endBatch();
				super.renderModel(blockRenderer, buffer, renderType,
						buffer.getBuffer(ItemBlockRenderTypes.getRenderType(blockState, false)), ms);
				Lighting.setupFor3DItems();
				return;
			}
			...
			FluidRenderer.renderFluidBox(new FluidStack(blockState.getFluidState().getType(), FluidConstants.BUCKET), 0,
					0, 0, 1, 1, 1, buffer, ms, LightTexture.FULL_BRIGHT, false);
			buffer.endBatch();
		}
	}
	...
	public static class GuiItemRenderBuilder extends GuiRenderBuilder {
		...
		public static void renderItemIntoGUI(PoseStack matrixStack, ItemStack stack, boolean useDefaultLighting) {
			...
			renderer.render(stack, ItemDisplayContext.GUI, false, matrixStack, buffer, LightTexture.FULL_BRIGHT,
