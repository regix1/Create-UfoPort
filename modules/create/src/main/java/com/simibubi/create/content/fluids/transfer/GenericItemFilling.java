...
public class GenericItemFilling {
	...
	/**
	 * Checks if an ItemStack's IFluidHandlerItem is valid. Ideally, this check
	 * would not be necessary. Unfortunately, some mods that copy the functionality
	 * of the MilkBucketItem copy the FluidBucketWrapper capability that is patched
	 * in by Forge without looking into what it actually does. In all cases this is
	 * incorrect because having a non-bucket item turn into a bucket item does not
	 * make sense.
	 *
	 * <p>
	 * This check is only necessary for filling since a FluidBucketWrapper will be
	 * empty if it is initialized with a non-bucket item.
	 *
	 * @param stack        The ItemStack.
	 * @param fluidHandler The IFluidHandlerItem instance retrieved from the
	 *                     ItemStack.
...
//		}
	...
	private static boolean canFillBucketInternally(FluidStack availableFluid) {
		return true; // fabric: this is false on forge for reasons I'm not entirely sure of. we need
						// it true here to catch buckets.
	}
