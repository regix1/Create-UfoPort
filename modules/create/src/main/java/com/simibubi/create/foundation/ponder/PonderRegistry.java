...
public class PonderRegistry {
	...
	public static List<PonderScene> compile(List<PonderStoryBoardEntry> entries) {
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			Level level = EnvExecutor.unsafeRunForDist(() -> () -> Minecraft.getInstance().level, () -> () -> {
			...
			activeTemplate.placeInWorld(world, BlockPos.ZERO, BlockPos.ZERO, settings, world.random,
