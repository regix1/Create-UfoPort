...
import io.github.fabricators_of_create.porting_lib_ufo.mixin.accessors.common.accessor.RecipeManagerAccessor;
...
public class CreateJEI implements IModPlugin {
	...
	public static <T extends Recipe<?>> void consumeTypedRecipes(Consumer<T> consumer, RecipeType<? extends Recipe> type) {
		Collection<RecipeHolder<?>> map = ((RecipeManagerAccessor) Minecraft.getInstance()
			.getConnection()
			.getRecipeManager()).port_lib$byType((RecipeType)type);
		if (map != null) {
			map.forEach(recipe -> consumer.accept((T) recipe.value()));
		}
	}
	...
	public static boolean doOutputsMatch(Recipe<?> recipe1, Recipe<?> recipe2) {
		RegistryAccess registryAccess = Create.getRegistryAccess();
