...
public class AllItems {
	...
	public static final ItemEntry<Item> RAW_ZINC =
		taggedIngredient("raw_zinc", forgeItemTag("raw_zinc_ores"), TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath("c", "raw_ores")));
	...
	public static final ItemEntry<GogglesItem> GOGGLES = REGISTRATE.item("goggles", GogglesItem::new)
		...
		.properties(p -> {
			p.equipmentSlot(GogglesItem::getEquipmentSlot);
			return p;
		})
		...
		REGISTRATE
			.item("copper_backtank",
				p -> new BacktankItem(
						...
						p.durability(ArmorItem.Type.CHESTPLATE.getDurability(20)),  
	...
	COPPER_DIVING_HELMET =
		REGISTRATE
			.item("copper_diving_helmet",
				p -> new DivingHelmetItem(
						AllArmorMaterials.COPPER, 
						p.durability(ArmorItem.Type.HELMET.getDurability(20)), 
						Create.asResource("copper_diving")))
	...
	COPPER_DIVING_BOOTS =
		REGISTRATE
			.item("copper_diving_boots",
				p -> new DivingBootsItem(
						AllArmorMaterials.COPPER, 
						p.durability(ArmorItem.Type.BOOTS.getDurability(20)), 
						Create.asResource("copper_diving")))
