...
public class AllCreativeModeTabs {
	...
	public static final TabInfo BASE_CREATIVE_TAB = register("base",
			() -> FabricItemGroup.builder().title(Components.translatable("itemGroup.create.base"))
					.icon(() -> AllBlocks.COGWHEEL.asStack())
					.displayItems(
							new RegistrateDisplayItemsGenerator(true, () -> AllCreativeModeTabs.BASE_CREATIVE_TAB))
					.build());

	public static final TabInfo PALETTES_CREATIVE_TAB = register("palettes",
			() -> FabricItemGroup.builder().title(Components.translatable("itemGroup.create.palettes"))
					.icon(() -> AllPaletteBlocks.ORNATE_IRON_WINDOW.asStack())
					.displayItems(
							new RegistrateDisplayItemsGenerator(false, () -> AllCreativeModeTabs.PALETTES_CREATIVE_TAB))
					.build());
	...
	private static class RegistrateDisplayItemsGenerator implements DisplayItemsGenerator {
		...
		private static Predicate<Item> makeExclusionPredicate() {
			...
			List<ItemProviderEntry<?>> simpleExclusions = List.of(AllItems.INCOMPLETE_PRECISION_MECHANISM,
					AllItems.INCOMPLETE_REINFORCED_SHEET, AllItems.INCOMPLETE_TRACK, AllItems.CHROMATIC_COMPOUND,
					...
					AllBlocks.MYSTERIOUS_CUCKOO_CLOCK, AllBlocks.ELEVATOR_CONTACT, AllBlocks.SHADOW_STEEL_CASING,
					AllBlocks.REFINED_RADIANCE_CASING);

			List<ItemEntry<TagDependentIngredientItem>> tagDependentExclusions = List.of(AllItems.CRUSHED_OSMIUM,
		...
		private static List<ItemOrdering> makeOrderings() {
			...
			Map<ItemProviderEntry<?>, ItemProviderEntry<?>> simpleBeforeOrderings = Map.of(AllItems.EMPTY_BLAZE_BURNER,
					AllBlocks.BLAZE_BURNER, AllItems.SCHEDULE, AllBlocks.TRACK_STATION);

			Map<ItemProviderEntry<?>, ItemProviderEntry<?>> simpleAfterOrderings = Map.of(AllItems.VERTICAL_GEARBOX,
					AllBlocks.GEARBOX);
		...
		private static Function<Item, ItemStack> makeStackFunc() {
			...
			Map<ItemProviderEntry<?>, Function<Item, ItemStack>> simpleFactories = Map.of(AllItems.COPPER_BACKTANK,
					item -> {
		...
		private static Function<Item, TabVisibility> makeVisibilityFunc() {
			...
			Map<ItemProviderEntry<?>, TabVisibility> simpleVisibilities = Map.of(AllItems.BLAZE_CAKE_BASE,
		...
		private static void outputAll(Output output, List<Item> items, Function<Item, ItemStack> stackFunc,
				Function<Item, TabVisibility> visibilityFunc) {
			for (Item item : items) {
