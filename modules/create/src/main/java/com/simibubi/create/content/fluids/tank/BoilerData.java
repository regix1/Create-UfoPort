...
public class BoilerData {
	...
	public boolean addToGoggleTooltip(List<Component> tooltip, boolean isPlayerSneaking, int boilerSize) {
		...
		if (attachedEngines > 0 && maxHeatForSize > 0 && maxHeatForWater == 0 && (passiveHeat ? 1 : activeHeat) > 0) {
			...
			Lang.number(waterSupply).style(ChatFormatting.BLUE).add(Lang.translate("generic.unit.millibuckets"))
					.add(Lang.text(" / ").style(ChatFormatting.GRAY))
					.add(Lang
							.translate("boiler.per_tick",
									Lang.number(waterSupplyPerLevel).add(Lang.translate("generic.unit.millibuckets")))
		...
		Lang.number(totalSU).translate("generic.unit.stress").style(ChatFormatting.AQUA).space()
	...
	private MutableComponent componentHelper(String label, int level, boolean forGoggles, boolean useBlocksAsBars,
		...
		return Lang.translateDirect("boiler." + label).withStyle(style1)
				.append(Lang.translateDirect("boiler." + label + "_dots").withStyle(style2)).append(base);
	}
	...
	private MutableComponent barComponent(int level) {
		return Components.empty().append(bars(Math.max(0, minValue - 1), ChatFormatting.DARK_GREEN))
