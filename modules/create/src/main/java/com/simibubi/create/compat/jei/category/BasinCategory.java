...
public class BasinCategory extends CreateRecipeCategory<BasinRecipe> {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, BasinRecipe recipe, IFocusGroup focuses) {
		...
		for (Pair<Ingredient, MutableInt> pair : condensedIngredients) {
			...
			builder.addSlot(RecipeIngredientRole.INPUT, 17 + xOffset + (i % 3) * 19, 51 - (i / 3) * 19)
					.setBackground(getRenderedSlot(), -1, -1).addItemStacks(stacks);
			i++;
		}
		for (FluidIngredient fluidIngredient : recipe.getFluidIngredients()) {
			builder.addSlot(RecipeIngredientRole.INPUT, 17 + xOffset + (i % 3) * 19, 51 - (i / 3) * 19)
					...
					.addIngredients(FabricTypes.FLUID_STACK,
							toJei(withImprovedVisibility(fluidIngredient.getMatchingFluidStacks())))
		...
		for (ProcessingOutput result : recipe.getRollableResults()) {
			...
			builder.addSlot(RecipeIngredientRole.OUTPUT, xPosition, yPosition)
					.setBackground(getRenderedSlot(result), -1, -1).addItemStack(result.getStack())
		...
		for (FluidStack fluidResult : recipe.getFluidResults()) {
			...
			builder.addSlot(RecipeIngredientRole.OUTPUT, xPosition, yPosition).setBackground(getRenderedSlot(), -1, -1)
	...
	@Override
	public void draw(BasinRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX,
			double mouseY) {
		HeatCondition requiredHeat = recipe.getRequiredHeat();
		...
		graphics.drawString(Minecraft.getInstance().font, Lang.translateDirect(requiredHeat.getTranslationKey()), 9, 86,
