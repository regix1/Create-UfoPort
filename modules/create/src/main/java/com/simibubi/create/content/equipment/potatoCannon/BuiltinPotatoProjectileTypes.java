...
public class BuiltinPotatoProjectileTypes {
	...
	private static Predicate<EntityHitResult> setFire(int seconds) {
		return ray -> {
			ray.getEntity().setRemainingFireTicks(seconds);
			return false;
		};
	}
	...
	private static Predicate<EntityHitResult> foodEffects(FoodProperties food, boolean recoverable) {
		return ray -> {
			...
			if (entity instanceof LivingEntity) {
				food.effects().forEach(eff -> {
					if (Create.RANDOM.nextFloat() < eff.probability())
						applyEffect((LivingEntity) entity, new MobEffectInstance(eff.effect()));
				});
			}
			return !recoverable;
		};
