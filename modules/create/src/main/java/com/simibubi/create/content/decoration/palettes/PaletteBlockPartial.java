...
public abstract class PaletteBlockPartial<B extends Block> {
	...
	public @NonnullType BlockBuilder<B, CreateRegistrate> create(String variantName, PaletteBlockPattern pattern,
		...
		BlockBuilder<B, CreateRegistrate> blockBuilder = Create.REGISTRATE.block(blockName, p -> createBlock(block))
	...
	private static class Slab extends PaletteBlockPartial<SlabBlock> {
		...
		protected void createRecipes(AllPaletteStoneTypes type, BlockEntry<? extends Block> patternBlock,
			...
			ingredient.getCritereon(p);
			ShapelessRecipeBuilder.shapeless(category, patternBlock.get()).requires(ingredient).requires(ingredient)
					.unlockedBy("has_" + c.getName(), TriggerInstance.hasItems(c.get()))
					.save(p.getCallback(), Create.ID + ":" + c.getName() + "_recycling");
		}
	...
	private static class Wall extends PaletteBlockPartial<WallBlock> {
		...
		protected ItemBuilder<BlockItem, BlockBuilder<WallBlock, CreateRegistrate>> transformItem(
				...
				PaletteBlockPattern pattern) {
			builder.model((c, p) -> p.wallInventory(c.getName(),
					getTexture(variantName, pattern, 0)));
		...
		protected void createRecipes(AllPaletteStoneTypes type, BlockEntry<? extends Block> patternBlock,
			...
			ingredient.getCritereon(p);
			ShapedRecipeBuilder.shaped(category, c.get(), 6).pattern("XXX").pattern("XXX").define('X', ingredient)
					.unlockedBy("has_" + p.safeName(ingredient), TriggerInstance.hasItems(patternBlock.get()))
					.save(p.getCallback(), p.safeId(c.get()));
		}
