...
public class ConfigModListScreen extends ConfigScreen {
	...
	protected void init() {
		...
		list.setX(this.width / 2 - ((AbstractSelectionListAccessor) list).port_lib$getWidth() / 2);
		addRenderableWidget(list);

		allEntries = new ArrayList<>();
		FabricLoader.getInstance().getAllMods()
				.forEach(container -> allEntries.add(new ModEntry(container.getMetadata().getId(), this)));
//		ModList.get().getMods().stream().map(IModInfo::getModId).forEach(id -> allEntries.add(new ModEntry(id, this)));
		...
		goBack.showingElement(
				AllIcons.I_CONFIG_BACK.asStencil().withElementRenderer(BoxWidget.gradientFactory.apply(goBack)));
	...
	private void updateFilter(String search) {
		list.children().clear();
		allEntries.stream().filter(modEntry -> modEntry.id.contains(search.toLowerCase(Locale.ROOT)))
	...
	public static class ModEntry extends ConfigScreenList.LabeledEntry {
		...
		public ModEntry(String id, Screen parent) {
			...
			button = new BoxWidget(0, 0, 35, 16).showingElement(AllIcons.I_CONFIG_OPEN.asStencil().at(10, 0));
			button.modifyElement(
					e -> ((DelegatedStencilElement) e).withElementRenderer(BoxWidget.gradientFactory.apply(button)));
			...
			if (ConfigHelper.hasAnyForgeConfig(id)) {
				...
				button.modifyElement(
						e -> ((DelegatedStencilElement) e).withElementRenderer(BaseConfigScreen.DISABLED_RENDERER));
				labelTooltip.add(Components.literal(toHumanReadable(id)));
				labelTooltip.addAll(TooltipHelper.cutStringTextComponent(
						"This Mod does not have any configs registered or is not using a compatible config system",
						Palette.ALL_GRAY));
			}
