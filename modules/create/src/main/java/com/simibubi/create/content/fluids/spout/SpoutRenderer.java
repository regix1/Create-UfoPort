...
public class SpoutRenderer extends SafeBlockEntityRenderer<SpoutBlockEntity> {
	...
	static final PartialModel[] BITS = { AllPartialModels.SPOUT_TOP, AllPartialModels.SPOUT_MIDDLE,
	...
	protected void renderSafe(SpoutBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		...
		if (processingTicks != -1) {
			...
			FluidRenderer.renderFluidBox(fluidStack, (float) bb.minX, (float) bb.minY, (float) bb.minZ, (float) bb.maxX,
					(float) bb.maxY, (float) bb.maxZ, buffer, ms, light, true);
		}
		...
		for (PartialModel bit : BITS) {
			CachedBufferer.partial(bit, be.getBlockState()).light(light).renderInto(ms,
