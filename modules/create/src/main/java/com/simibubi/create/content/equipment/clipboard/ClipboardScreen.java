...
public class ClipboardScreen extends AbstractSimiScreen {
	...
	public void tick() {
		...
		int mx = (int) (this.minecraft.mouseHandler.xpos() * (double) this.minecraft.getWindow().getGuiScaledWidth()
				/ (double) this.minecraft.getWindow().getScreenWidth());
		int my = (int) (this.minecraft.mouseHandler.ypos() * (double) this.minecraft.getWindow().getGuiScaledHeight()
		...
		if (mx > 0 && mx < 183 && my > 0 && my < 190) {
			...
			for (int i = 0; i < currentEntries.size(); i++) {
				...
				totalHeight += Math.max(12,
						font.split(Components.literal(text), clipboardEntry.icon.isEmpty() ? 150 : 130).size() * 9 + 3);
	...
	private boolean validateTextForEntry(String newText) {
		...
		for (int i = 0; i < currentEntries.size(); i++) {
			...
			totalHeight += Math.max(12, font.split(Components.literal(text), 150).size() * 9 + 3);
		}
	...
	protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < currentEntries.size(); i++) {
			...
			for (FormattedCharSequence sequence : split) {
				if (i != editingIndex)
					graphics.drawString(font, sequence, x + 58 + iconOffset, y + 50, checked ? 0x31B25D : 0x311A00,
							false);
				y += 9;
			}
			y += 3;
	...
	@Override
	public boolean mouseScrolled(double pMouseX, double pMouseY, double amount, double pDelta) {
		changePage(pDelta < 0);
		return true;
	}
	...
	public boolean charTyped(char pCodePoint, int pModifiers) {
		if (super.charTyped(pCodePoint, pModifiers))
			return true;
		if (!isAllowedChatCharacter(pCodePoint))
			return false;
		if (editingIndex == -1)
			return false;
	...
	private static boolean isAllowedChatCharacter(char character) {
			...
			case 335:
				if (hasShiftDown()) {
					...
					if (currentEntries.size() <= editingIndex + 1
							|| !currentEntries.get(editingIndex + 1).text.getString().isEmpty())
...
//		RenderSystem.enableTexture();
	...
	static class LineInfo {
		...
		public LineInfo(Style pStyle, String pContents, int pX, int pY) {
			...
			x = pX;
			y = pY;
			asComponent = Components.literal(pContents).setStyle(pStyle);
		}
	}
