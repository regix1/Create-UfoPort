package io.github.fabricators_of_create.porting_lib_ufo.util.client;
...
public class ScreenUtils {
	...
	public static void drawGradientRect(Matrix4f matrix, int z, int left, int top, int right, int bottom,
		...
		buffer.addVertex(matrix, right, bottom, z).setColor(endRed, endGreen, endBlue, endAlpha);
		...
		if (!textLines.isEmpty()) {
			...
			if (needsWrap) {
				...
				for (int i = 0; i < textLines.size(); i++) {
					...
					List<FormattedText> wrappedLine = font.getSplitter().splitLines(textLine, tooltipTextWidth,
							Style.EMPTY);
					if (i == 0)
			...
			MultiBufferSource.BufferSource renderType = MultiBufferSource
					.immediate(new ByteBufferBuilder(256));
			...
			for (int lineNumber = 0; lineNumber < textLines.size(); ++lineNumber) {
				...
				if (line != null)
					font.drawInBatch(Language.getInstance().getVisualOrder(line), (float) tooltipX, (float) tooltipY,
							-1, true, mat, renderType, Font.DisplayMode.NORMAL, 0, 15728880);
	 ...
	 * based on a fixed size textured box with continuous borders and filler. The
	 * provided ResourceLocation object will be bound using
	 * Minecraft.getMinecraft().getTextureManager().bindTexture(resourceLocation).
	 *
	 * @param guiGraphics   the gui pose stack
	 * @param res           the ResourceLocation object that contains the desired
	 *                      image
	 ...
	 * @param textureWidth  the width of the box texture in the resource location
	 *                      image
	 * @param textureHeight the height of the box texture in the resource location
	 *                      image
	 ...
	 */
	public static void drawContinuousTexturedBox(GuiGraphics guiGraphics, ResourceLocation res, int x, int y, int u,
			int v, int width, int height, int textureWidth, int textureHeight, int topBorder, int bottomBorder,
			int leftBorder, int rightBorder, float zLevel) {
		RenderSystem.setShader(GameRenderer::getPositionTexShader);
		RenderSystem.setShaderTexture(0, res);
		drawContinuousTexturedBox(guiGraphics, x, y, u, v, width, height, textureWidth, textureHeight, topBorder,
				bottomBorder, leftBorder, rightBorder, zLevel);
	}
	 ...
	 * @param zLevel        the zLevel to draw at
	 */
	public static void drawContinuousTexturedBox(GuiGraphics guiGraphics, int x, int y, int u, int v, int width,
		...
		// Top Right
		drawTexturedModalRect(guiGraphics, x + leftBorder + canvasWidth, y, u + leftBorder + fillerWidth, v,
				rightBorder, topBorder, zLevel);
		// Bottom Left
		drawTexturedModalRect(guiGraphics, x, y + topBorder + canvasHeight, u, v + topBorder + fillerHeight, leftBorder,
				bottomBorder, zLevel);
		// Bottom Right
		drawTexturedModalRect(guiGraphics, x + leftBorder + canvasWidth, y + topBorder + canvasHeight,
				u + leftBorder + fillerWidth, v + topBorder + fillerHeight, rightBorder, bottomBorder, zLevel);

		for (int i = 0; i < xPasses + (remainderWidth > 0 ? 1 : 0); i++) {
			// Top Border
			drawTexturedModalRect(guiGraphics, x + leftBorder + (i * fillerWidth), y, u + leftBorder, v,
					(i == xPasses ? remainderWidth : fillerWidth), topBorder, zLevel);
			// Bottom Border
			drawTexturedModalRect(guiGraphics, x + leftBorder + (i * fillerWidth), y + topBorder + canvasHeight,
					u + leftBorder, v + topBorder + fillerHeight, (i == xPasses ? remainderWidth : fillerWidth),
					bottomBorder, zLevel);

			// Throw in some filler for good measure
			for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++)
				drawTexturedModalRect(guiGraphics, x + leftBorder + (i * fillerWidth),
		...
		for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++) {
			// Left Border
			drawTexturedModalRect(guiGraphics, x, y + topBorder + (j * fillerHeight), u, v + topBorder, leftBorder,
					(j == yPasses ? remainderHeight : fillerHeight), zLevel);
			// Right Border
			drawTexturedModalRect(guiGraphics, x + leftBorder + canvasWidth, y + topBorder + (j * fillerHeight),
					u + leftBorder + fillerWidth, v + topBorder, rightBorder,
					(j == yPasses ? remainderHeight : fillerHeight), zLevel);
		}
	}

	public static void drawTexturedModalRect(GuiGraphics guiGraphics, int x, int y, int u, int v, int width, int height,
			float zLevel) {
