package io.github.fabricators_of_create.porting_lib_ufo.transfer.fluid.item;
...
public class FluidHandlerItemStack implements Storage<FluidVariant> {
	...
	public static final DataComponentType<FluidStack> FLUID_DATA = register("portlib_fluid_data", 
			builder -> builder.persistent(FluidStack.CODEC).networkSynchronized(FluidStack.STREAM_CODEC).cacheEncoding());
	...
	public FluidStack getFluid() {
		Optional<? extends FluidStack> opt = container.getItemVariant().getComponents().get(FLUID_DATA);
		if(opt.isEmpty()) {
