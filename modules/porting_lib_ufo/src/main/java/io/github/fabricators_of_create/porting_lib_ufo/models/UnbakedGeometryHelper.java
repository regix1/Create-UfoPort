...
import io.github.fabricators_of_create.porting_lib_ufo.models.geometry.SimpleModelState;
...
public class UnbakedGeometryHelper {
	...
	public static void bakeElements(List<BakedQuad> quads, List<BlockElement> elements, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState) {
		for (BlockElement element : elements) {
			element.faces.forEach((side, face) -> {
				var sprite = spriteGetter.apply(new Material(TextureAtlas.LOCATION_BLOCKS, ResourceLocation.parse(face.texture())));
				quads.add(BlockModel.FACE_BAKERY.bakeQuad(element.from, element.to, face, sprite, side, modelState, element.rotation, element.shade));
			});
		}
	}
	...
	public static List<BakedQuad> bakeElements(List<BlockElement> elements, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState) {
		...
		bakeElements(list, elements, spriteGetter, modelState);
		return list;
	}
