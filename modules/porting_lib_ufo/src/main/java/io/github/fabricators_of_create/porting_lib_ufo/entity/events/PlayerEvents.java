package io.github.fabricators_of_create.porting_lib_ufo.entity.events;
...
public abstract class PlayerEvents extends LivingEntityEvents {
	...
	public static final Event<PlayerBreakSpeed> BREAK_SPEED = EventFactory.createArrayBacked(PlayerBreakSpeed.class,
			callbacks -> event -> {
				for (PlayerBreakSpeed e : callbacks)
					e.setBreakSpeed(event);
			});

	public static final Event<PlayerXpPickUp> PICKUP_XP = EventFactory.createArrayBacked(PlayerXpPickUp.class,
			callbacks -> event -> {
				for (PlayerXpPickUp e : callbacks)
					e.onPlayerPicksUpXp(event);
			});

	public static final Event<PlayerXpChange> XP_CHANGE = EventFactory.createArrayBacked(PlayerXpChange.class,
			callbacks -> event -> {
				for (PlayerXpChange e : callbacks)
					e.onPlayerXpChange(event);
			});

	public static final Event<PlayerLoggedInOrOut> LOGGED_IN = EventFactory.createArrayBacked(PlayerLoggedInOrOut.class,
			callbacks -> player -> {
				for (PlayerLoggedInOrOut e : callbacks)
					e.handleConnection(player);
			});

	public static final Event<PlayerLoggedInOrOut> LOGGED_OUT = EventFactory
			.createArrayBacked(PlayerLoggedInOrOut.class, callbacks -> player -> {
				for (PlayerLoggedInOrOut e : callbacks)
					e.handleConnection(player);
			});
	 ...
	 * {@link #originalSpeed} contains the original speed at which the player broke
	 * the block. <br>
	 * {@link #newSpeed} contains the newSpeed at which the player will break the
	 * block. <br>
	 * {@link #pos} contains the coordinates at which this event is occurring.
	 * Optional value.<br>
	 * <br>
