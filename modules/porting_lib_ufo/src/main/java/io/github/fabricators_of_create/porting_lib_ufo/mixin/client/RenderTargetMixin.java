...
import io.github.fabricators_of_create.porting_lib_ufo.extensions.extensions.RenderTargetExtensions;
...
public abstract class RenderTargetMixin implements RenderTargetExtensions {
	...
	private boolean port_lib$stencilBuffer(int i, int j, int k, int l, int m) {
		if (port_lib$stencilEnabled) {
			GlStateManager._glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_DEPTH_ATTACHMENT, 3553,
					this.depthBufferId, 0);
			GlStateManager._glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_STENCIL_ATTACHMENT, 3553,
					this.depthBufferId, 0);
			return false;
		}
		return true;
	...
	// separate ModifyArg injects instead of one ModifyArgs to avoid creating
	// objects
