package io.github.fabricators_of_create.porting_lib_ufo.event.common;
	...
	public static class BreakEvent extends BlockEvents {
		...
		public BreakEvent(Level world, BlockPos pos, BlockState state, Player player) {
			...
			if (state == null || !PortingHooks.isCorrectToolForDrops(state, player)) { // Handle empty block or player unable to break block scenario
				...
				int bonusLevel = EnchantmentHelper.getItemEnchantmentLevel(PortingLibBase.getRegistryAccess().lookup(Registries.ENCHANTMENT).get()
						.getOrThrow(Enchantments.FORTUNE), player.getMainHandItem());
				int silklevel = EnchantmentHelper.getItemEnchantmentLevel(PortingLibBase.getRegistryAccess().lookup(Registries.ENCHANTMENT).get().
						getOrThrow(Enchantments.SILK_TOUCH), player.getMainHandItem());
