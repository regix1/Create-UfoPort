...
public abstract class ModdedEntityLootSubProvider extends EntityLootSubProvider {
	...
	public ModdedEntityLootSubProvider(FeatureFlagSet enabledFeatures, HolderLookup.Provider prov) {
		this(enabledFeatures, enabledFeatures, prov);
	}

	public ModdedEntityLootSubProvider(FeatureFlagSet allowed, FeatureFlagSet required, HolderLookup.Provider prov) {
		super(allowed, required, prov);
		...
		this.getKnownEntityTypes().map(EntityType::builtInRegistryHolder).forEach((entityTypeReference) -> {
			...
			if (entitytype.isEnabled(this.allowed)) {
				if (canHaveLootTable(entitytype)) {
					...
					if (map1 != null) {
						throw new IllegalStateException(String.format(Locale.ROOT, "Weird loottables '%s' for '%s', not a LivingEntity so should not have loot", map1.keySet().stream().map(ResourceKey::toString).collect(Collectors.joining(",")), entityTypeReference.key().location()));
					}
				}
