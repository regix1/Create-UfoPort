package io.github.fabricators_of_create.porting_lib_ufo.common.util;
...
public final class PlantType {
	private static final Pattern INVALID_CHARACTERS = Pattern.compile("[^a-z_]"); // Only a-z and _ are allowed, meaning
																					// names must be lower case. And use
																					// _ to separate words.
	...
	/**
	 * Getting a custom {@link PlantType}, or an existing one if it has the same
	 * name as that one. Your plant should implement {@link IPlantable} and return
	 * this custom type in {@link IPlantable#getPlantType(BlockGetter, BlockPos)}.
	 *
	 * <p>
	 * If your new plant grows on blocks like any one of them above, never create a
	 * new {@link PlantType}. This Type is only functioning in
	 * {@link Block#canSustainPlant(BlockState, BlockGetter, BlockPos, Direction, IPlantable)},
	 * which you are supposed to override this function in your new block and create
	 * a new plant type to grow on that block.
	 * <p>
	 * This method can be called during parallel loading
	 *
	 * @param name the name of the type of plant, you had better follow the style
	 *             above
