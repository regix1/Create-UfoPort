...
public class BlockSnapshot {
	...
	private static CompoundTag getBlockEntityTag(@Nullable BlockEntity te) {
		return te == null ? null : te.saveWithFullMetadata(PortingLibBase.getRegistryAccess());
	}
	...
	public BlockEntity getBlockEntity() {
		return getTag() != null ? BlockEntity.loadStatic(getPos(), getReplacedBlock(), getTag(), PortingLibBase.getRegistryAccess()) : null;
	}
	...
	public boolean restoreToLocation(LevelAccessor world, BlockPos pos, boolean force, boolean notifyNeighbors) {
		...
		if (getTag() != null) {
			...
			if (te != null) {
				te.loadCustomOnly(nbt, PortingLibBase.getRegistryAccess());
				te.setChanged();
			}
		}
