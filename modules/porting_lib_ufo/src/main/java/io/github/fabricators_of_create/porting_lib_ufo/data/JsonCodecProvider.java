...
import io.github.fabricators_of_create.porting_lib_ufo.core.util.LamdbaExceptionUtils;
...
public class JsonCodecProvider<T> implements DataProvider {
	...
	protected Map<ResourceLocation, ResourceCondition[]> conditions = Collections.emptyMap();
	...
	public CompletableFuture<?> run(final CachedOutput cache) {
		...
		gather(LamdbaExceptionUtils.rethrowBiConsumer((id, value) -> {
			...
			ResourceCondition[] conditions = this.conditions.get(id);
			if (conditions != null && conditions.length > 0) {
				if(encoded instanceof JsonObject obj) {
					JsonArray arr = new JsonArray();
					for(ResourceCondition iCond : conditions) {
						arr.add(ResourceCondition.CODEC.encode(iCond, JsonOps.INSTANCE, null).getOrThrow());
					}
	 ...
	 */
	public JsonCodecProvider<T> setConditions(Map<ResourceLocation, ResourceCondition[]> conditions) {
